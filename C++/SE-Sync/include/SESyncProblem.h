 ...
 * y = (Q - Lambda + sigma*I) x */
    struct QMinusLambdaProdFunctor {
        ...
        void perform_op(double* x, double* y) const
        {
            ...
            for (unsigned int i = 0; i < _problem->num_poses(); i++)
                Y.segment(i * _dim, _dim) -= _Lambda_blocks.block(0, i * _dim, _dim, _dim) * X.segment(i * _dim, _dim);
    ...
    /** Hacky workaround: store the gradient norm tolerance for the RTR optimization here, so that we can access it through a plain C-style callback that passes an ROPTLIB::Problem* instance as an argument */
