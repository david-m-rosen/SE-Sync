...
read_g2o_file(const std::string& filename, size_t& num_poses)
{
    ...
    while (std::getline(infile, line)) {
       ...
       */
            ...
            // Extract formatted output
            strstrm >> i >> j >> dx >> dy >> dz >> dqx >> dqy >> dqz >> dqw >> I11 >> I12 >> I13 >> I14 >> I15 >> I16 >> I22 >> I23 >> I24 >> I25 >> I26 >> I33 >> I34 >> I35 >> I36 >> I44 >> I45 >> I46 >> I55 >> I56 >> I66;
...
void construct_B_matrices(const std::vector<RelativePoseMeasurement>& measurements, Eigen::SparseMatrix<double>& B1, Eigen::SparseMatrix<double>& B2, Eigen::SparseMatrix<double>& B3)
{
    ...
    size_t max_pair; // Used for keeping track of the maximum pose that we've encountered so far
    ...
    for (unsigned int e = 0; e < measurements.size(); e++) {
        ...
        for (unsigned int l = 0; l < d; l++) {
            triplets.emplace_back(e * d + l, i * d + l, -sqrttau); // Diagonal element corresponding to tail
            triplets.emplace_back(e * d + l, j * d + l, sqrttau); // Diagonal element corresponding to head
        }
    ...
    for (unsigned int e = 0; e < measurements.size(); e++) {
        ...
        for (unsigned int k = 0; k < d; k++)
            for (unsigned int r = 0; r < d; r++)
                triplets.emplace_back(d * e + r, d2 * i + d * k + r, measurements[e].t(k));
    }
    ...
    for (unsigned int e = 0; e < measurements.size(); e++) {
        ...
        for (unsigned int r = 0; r < d; r++)
            for (unsigned int c = 0; c < d; c++) {
                ...
                for (unsigned int l = 0; l < d; l++)
                    triplets.emplace_back(e * d2 + d * r + l, i * d2 + d * c + l, -sqrtkappa * R(c, r));
            }
...
Eigen::MatrixXd chordal_initialization(unsigned int d, const Eigen::SparseMatrix<double>& B3)
{
    ...
    /// || B3 * r ||
    ///
    /// For the purposes of initialization, we can simply fix the first pose to the origin; this corresponds to fixing the first d^2 elements of r to vec(I_d), and slicing off the first d^2 columns of B3 to form
    ///
    ...
    for (unsigned int i = 1; i < num_poses; i++)
        Rchordal.block(0, i * d, d, d) = project_to_SOd(Rchordal.block(0, i * d, d, d));
    return Rchordal;
}
...
Eigen::MatrixXd recover_translations(const Eigen::SparseMatrix<double>& B1, const Eigen::SparseMatrix<double>& B2, const Eigen::MatrixXd& R)
{
    ...
    /// || B1 * t + B2 * vec(R) ||
    ///
    /// For the purposes of initialization, we can simply fix the first pose to the origin; this corresponds to fixing the first d elements of t to 0, and slicing off the first d columns of B1 to form
    ///
...
Eigen::MatrixXd round_solution(const Eigen::MatrixXd& Y, unsigned int d)
{
    ...
    Eigen::DiagonalMatrix<double, Eigen::Dynamic>::DiagonalVectorType& diagonal = Xid.diagonal();
    ...
    ROPTLIB::ProductElement& product_element)
{
    ...
    for (unsigned int j = 0; j < n; j++) {
        double* element_data = product_element.GetElement(j)->ObtainWriteEntireData();
