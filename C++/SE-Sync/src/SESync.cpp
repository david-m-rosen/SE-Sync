...
/** Helper function; trigger the stopping criterion based upon the gradient norm tolerance stored in the SESyncProblem object*/
bool gradient_norm_stopping_criterion(ROPTLIB::Variable* x, ROPTLIB::Vector* gf, double f, double ngf, double ngf0, const ROPTLIB::Problem* problem, const ROPTLIB::Solvers* solver)
{
    const SESyncProblem* SESync_problem_ptr = static_cast<const SESyncProblem*>(problem);
    ...
    const SESyncOpts& options, const Eigen::MatrixXd& Y0)
{
    ...
    SparseMatrix B1, B2, B3; // The measurement matrices B1, B2, B3 defined in equations (69) of the tech report
    ...
    if (options.verbose) {
        ...
        std::cout << " Using " << (options.use_Cholesky ? "Cholseky" : "QR") << " decomposition to compute orthogonal projections" << std::endl;
        std::cout << " Initialization method: " << (options.use_chordal_initialization ? "chordal" : "random") << std::endl
                  << std::endl;
    ...
    auto LGrho_counter = std::chrono::high_resolution_clock::now() - LGrho_start_time;
    double LGrho_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(LGrho_counter).count() / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << LGrho_elapsed_time << " seconds" << std::endl;
    ...
    double A_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(A_counter).count() / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << A_elapsed_time << " seconds" << std::endl;
    ...
    auto Omega_counter = std::chrono::high_resolution_clock::now() - Omega_start_time;
    double Omega_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(Omega_counter)
                                    .count()
        / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << Omega_elapsed_time << " seconds"
                  << std::endl;
    ...
    double T_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(T_counter).count() / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << T_elapsed_time << " seconds" << std::endl;
    ...
    double B_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(B_counter).count() / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << B_elapsed_time << " seconds" << std::endl
                  << std::endl;
    ...
    auto problem_construction_start_time = std::chrono::high_resolution_clock::now();
    SESyncProblem problem(LGrho, A, T, Omega, options.use_Cholesky);
    problem.RTR_gradient_norm_tolerance = options.tolgradnorm;
    auto problem_construction_counter = std::chrono::high_resolution_clock::now() - problem_construction_start_time;
    auto problem_construction_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(problem_construction_counter).count() / 1000.0;
    if (options.verbose)
        std::cout << "elapsed computation time: " << problem_construction_elapsed_time
                  << " seconds" << std::endl
                  << std::endl;
    ...
    if (Y0.size() != 0) {
        ...
        assert((Y0.rows() == options.r0) && (Y0.cols() == problem.dimension() * problem.num_poses()));
        ...
        if (options.use_chordal_initialization) {
            ...
            // Matrix Rinit = chordal_initialization(LGrho, problem.dimension(),options.max_eig_iterations, 100 * options.eig_comp_tol);
            Matrix Rinit = chordal_initialization(problem.dimension(), B3);
            auto chordal_init_counter = std::chrono::high_resolution_clock::now() - chordal_init_start_time;
            double chordal_init_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                                                   chordal_init_counter)
                                                   .count()
                / 1000.0;
            if (options.verbose)
                std::cout << "elapsed computation time: " << chordal_init_elapsed_time << " seconds"
                          << std::endl;
    ...
    for (unsigned int r = options.r0; r <= options.rmax; r++) {
        ...
        // Note that this custom stopping criterion is called before, and IN ADDITION TO, the relative function decrease tolerance; thus, by setting both, we enforce stopping base both upon gradient norm AND relative function decrease
        ...
        // Extract the results
        const ROPTLIB::ProductElement* Yopt_ropt = static_cast<const ROPTLIB::ProductElement*>(RTR.GetXopt());
        ...
        if (options.verbose) {
            ...
            std::cout << std::endl
                      ...
                      << results.SDPval << "!  Elapsed computation time: " << RTR.GetComTime() << " seconds" << std::endl
                      << std::endl;
        ...
        bool eigenvalue_convergence = problem.compute_Q_minus_Lambda_min_eig(results.Yopt, results.lambda_min, results.v_min, options.max_eig_iterations, options.eig_comp_tol, options.num_Lanczos_vectors);
        auto eig_counter = std::chrono::high_resolution_clock::now() - eig_start_time;
        double eig_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(eig_counter).count() / 1000.0;

        /// Tests for eigenvalue precision
        if (!eigenvalue_convergence) {
            std::cout << "WARNING!  EIGENVALUE COMPUTATION DID NOT CONVERGE TO DESIRED PRECISION!" << std::endl;
        ...
        if (results.lambda_min > -options.min_eig_num_tol) {
            ...
            /// ESCAPE FROM SADDLE!
            ///
            /// We will perform a backtracking line search along the direction of the negative eigenvector to escape from the saddle point

            // Set the initial step length to 100 times the distance needed to arrive at a trial point whose gradient is large enough to avoid triggering the RTR gradient norm tolerance stopping condition, according to the local second-order model
            double alpha = 2 * 100 * options.tolgradnorm / fabs(results.lambda_min);
            double alpha_min = 1e-6; // Minimum stepsize

            //            // First, double-check that the eigenvalue computation converged with sufficient accuracy ...
            //            Eigen::VectorXd QminusLambda_vmin(results.v_min.size());
            //            SESyncProblem::QMinusLambdaProdFunctor QminusLambda(&problem, results.Yopt);
            //            QminusLambda.perform_op(results.v_min.data(), QminusLambda_vmin.data());
            //            double rayleigh_quotient = results.v_min.dot(QminusLambda_vmin);

            //            // If the rayleigh quotient has the wrong sign, or if the Rayleigh quotient is more than an order of magnitude off of the estimate, the eigenvector is likely not accurate enough to enable us to escape the saddle
            //            if (rayleigh_quotient > 0 || fabs(rayleigh_quotient / results.lambda_min) < .1) {
            //                results.status = EIG_IMPRECISION;
            //                if (options.verbose) {
            //                    std::cout << "WARNING!  Eigencomputation is not sufficiently accurate to enable escape from saddle point!" << std::endl;
            //                    std::cout << "Estimated eigenvalue: " << results.lambda_min << std::endl;
            //                    std::cout << "Rayleigh quotient: " << rayleigh_quotient << std::endl
            //                              << std::endl;
            //                }
            //                break;
            //            }
       ...
       * and provides a direction ofnegative curvature */
            ...
            Eigen::MatrixXd Ydot = Eigen::MatrixXd::Zero(r + 1, problem.num_poses() * problem.dimension());
            ...
            do {
                ...
                Eigen::MatrixXd YtestMat(r + 1, problem.num_poses() * problem.dimension());
                StiefelProd2Mat(Ytest, YtestMat);

                // Ensure that the trial point Ytest has a lower function value than the current iterate Xprod, and that the gradient at Ytest is sufficiently negative that we will not automatically trigger the gradient tolerance stopping criterion at the next iteration
                double FYtest = problem.f(&Ytest);
                double FYtest_gradnorm = 2 * problem.Q_product(YtestMat.transpose()).norm();

                if ((FYtest < results.SDPval) && (FYtest_gradnorm > options.tolgradnorm))
                    escape_success = true;
            ...
            if (escape_success) {
                ...
                std::cout << "WARNING!  BACKTRACKING LINE SEARCH FAILED TO ESCAPE FROM SADDLE POINT!" << std::endl;
    ...
    if (options.verbose) {
        ...
        case EIG_IMPRECISION:
            std::cout << "WARNING: Minimum eigenvalue computation did not achieve sufficient accuracy; solution may not be globally optimal!" << std::endl;
            break;
        case SADDLE_POINT:
            std::cout << "WARNING: Line-search was unable to escape saddle point!" << std::endl;
            break;
        case RS_ITER_LIMIT:
            std::cout << "WARNING:  Riemannian Staircase reached the maximum level before finding global optimum!" << std::endl;
            break;
        }
    }
    ...
    auto rounding_counter = std::chrono::high_resolution_clock::now() - rounding_start_time;
    double rounding_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(rounding_counter).count() / 1000.0;

    if (options.verbose)
        std::cout << "elapsed computation time: " << rounding_elapsed_time << " seconds" << std::endl;

    results.Fxhat = (results.Rhat * problem.Q_product(results.Rhat.transpose())).trace();

    if (options.verbose)
        std::cout << "Recovering translational state estimates ... ";

    auto translations_recovery_start_time = std::chrono::high_resolution_clock::now();
    results.that = recover_translations(B1, B2, results.Rhat);
    auto translations_recovery_counter = std::chrono::high_resolution_clock::now() - translations_recovery_start_time;
    double translations_recovery_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(translations_recovery_counter).count() / 1000.0;

    if (options.verbose)
        std::cout << "elapsed computation time: " << translations_recovery_elapsed_time << " seconds" << std::endl
                  << std::endl;

    auto SESync_counter = std::chrono::high_resolution_clock::now() - SESync_start_time;
    double SESync_elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(SESync_counter).count() / 1000.0;

    if (options.verbose) {
        std::cout << "Value of SDP solution F(Y): " << results.SDPval << std::endl;
        std::cout << "Norm of Riemannian gradient grad F(Y): " << results.gradnorm
                  << std::endl;
        std::cout << "Minimum eigenvalue of Q - Lambda(Y): " << results.lambda_min << std::endl;
